...
d存储位置为[14~15]，大小为2字节
e存储位置为[16~19]，大小为4字节
str存储位置为[20~43]，大小为24字节，这个是上面输出已知的。
```
按照上面的计算```struct3```的sizeof应该是44，但是还有一个计算规则，那就是总体大小必须是最大成员的整数倍，这个结构体中最大成员是```double```类型的成员其大小为8字节，所以需要补齐，最后得到48。然后我们继续看下面结构体：
...
d存储位置为[14~15]，大小为2字节
e存储位置为[16~19]，大小为4字节
str存储位置为[20~35]，大小为16字节，这个是上面输出已知的。
```
按照上面的计算```struct4```的sizeof应该是36，但是还有一个计算规则，那就是总体大小必须是最大成员的整数倍，这个结构体中最大成员是```double```类型的成员其大小为8字节，所以需要补齐，最后得到40。
